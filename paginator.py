...
with open('paginator.csv', 'w', newline='') as file:
    writer = csv.writer(file)

    # Write header row to the CSV file
    header = ["Auto Scaling Group", "Instance Count", "AMI ID", "AMI Creation Date", "Instance Refresh Date"]
        ...
        if 'LaunchTemplate' in asg:
            ...
            if launch_template_versions:
                ...
                if ami_images:
                    ...
                    instance_refresh_date = None
                    if 'InstanceRefreshes' in asg:
                        for instance_refresh in asg['InstanceRefreshes']:
                            if 'InstanceRefreshId' in instance_refresh:
                                refresh_id = instance_refresh['InstanceRefreshId']
                                refresh_response = asg_client.describe_instance_refreshes(
                                    AutoScalingGroupName=group_name,
                                    InstanceRefreshIds=[refresh_id]
                                )
                                if 'InstanceRefreshes' in refresh_response and refresh_response['InstanceRefreshes']:
                                    instance_refresh_date = refresh_response['InstanceRefreshes'][0]['Status']['InstanceRefreshStatus'].get('StatusUpdateTime')

                    # Write the Auto Scaling Group summary to the CSV file
                    row = [group_name, str(instance_count), ami_id, ami_creation_date, instance_refresh_date]
                    writer.writerow(row)
                else:
